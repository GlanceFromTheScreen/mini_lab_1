...
class Entries:
    ...
    def add_entry(self):
        ...
        new_entry.icursor(0)
        new_entry.bind('<FocusIn>', self.take_FocusIn)
        new_entry.focus()
        new_entry.pack()
...
class Commands:
    ...
    def delete_focus(self, *args, **kwargs):
        ...
        if (len(focus_entry.get()) == 0):
            ...
            modal_w = ModalWindow(self.parent_window, title="Warning", labeltext="Удаляется непустое поле")
            leave_button = Button(master=modal_w.top, text='Ок', command=modal_w.cancel)
